...
export const ChatInterface: React.FC<ChatInterfaceProps> = ({ activeSessionId, activeSession, loadSessions }) => {
  ...
  const userContext = useMemo((): UserProfileContext => ({
    ...
    interests,
    background,
    contacts: ecosystemStore.contacts,
    events: ecosystemStore.events,
    communities: ecosystemStore.communities,
    organizations: ecosystemStore.organizations,
    skills: ecosystemStore.skills,
    projects: ecosystemStore.projects,
    knowledge: ecosystemStore.knowledge,
    relationships: ecosystemStore.relationships,
    criticalPaths: ecosystemStore.criticalPaths,
    activeCriticalPathId: ecosystemStore.activeCriticalPathId,
  }), [goals, interests, background, ecosystemStore]);
  ...
  const handleCriticalPathSelect = (pathId: string) => {
    ecosystemStore.setActiveCriticalPath(pathId);
    const selectedPath = ecosystemStore.criticalPaths.find(p => p.id === pathId);
    toast.success("Critical Path set!", {
      description: `Your conversation context is now focused on "${selectedPath?.title}".`
    });
  };
  ...
  const renderContent = () => {
    ...
    if (chatState.messages.length === 0 && !chatState.isProcessing) {
      return <WelcomeScreen onCriticalPathSelect={handleCriticalPathSelect} />;
    }
    return (
