...
import { User, Calendar, Users, Building, GitBranch, BrainCircuit, Briefcase, Library, Flag } from 'lucide-react';
    ...
    >
      <div
        className={cn(
          ...
          hasData ? 'bg-primary/10 border-primary text-primary animate-glow' : 'bg-muted text-muted-foreground border-border',
...
export const EcosystemGraph: React.FC<EcosystemGraphProps> = ({ highlightedNodes }) => {
  ...
  const criticalPathControls = useAnimationControls();
  ...
  const animationControlsMap = useMemo(() => ({
    ...
    criticalPath: criticalPathControls,
  }), [youControls, contactsControls, eventsControls, communitiesControls, projectsControls, skillsControls, organizationsControls, knowledgeControls, criticalPathControls]);
  useEffect(() => {
  ...
  const nodes = [
    ...
    { id: 'knowledge', icon: <Library className="w-8 h-8" />, label: 'Knowledge', position: 'top-1/2 -left-8 -translate-y-1/2', hasData: ecosystem.knowledge.length > 0, controls: knowledgeControls },
    { id: 'criticalPath', icon: <Flag className="w-8 h-8" />, label: 'Critical Path', position: 'top-1/2 -right-8 -translate-y-1/2', hasData: !!ecosystem.activeCriticalPathId, controls: criticalPathControls },
  ];
