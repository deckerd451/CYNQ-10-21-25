...
export const EcosystemNodeDetail: React.FC<EcosystemNodeDetailProps> = ({ nodeType }) => {
  ...
  const { contacts, events, communities, organizations, skills, projects, knowledge, criticalPaths, activeCriticalPathId } = useEcosystemStore();
  const renderContent = () => {
    switch (nodeType) {
      case 'you':
        return (
      ...
      case 'criticalPath': {
        const activePath = criticalPaths.find(p => p.id === activeCriticalPathId);
        return (
          <DetailSection title="Active Critical Path" icon={<Flag className="w-4 h-4" />} isEmpty={!activePath}>
            {activePath && (
              <div className="space-y-2">
                <p className="font-bold text-sm">{activePath.title}</p>
                ...
                <div className="flex items-center text-xs text-muted-foreground pt-1 gap-2">
                  ...
                  <span>{activePath.overallTimeline}</span>
                </div>
              </div>
            )}
          </DetailSection>
