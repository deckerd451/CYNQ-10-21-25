...
import { Link, X, Users, Calendar, Building, Target, Sparkles, BookOpen, Briefcase, BrainCircuit, GitBranch, Library, Flag } from 'lucide-react';
...
const iconMap: { [key: string]: React.ReactNode } = {
  ...
  criticalPath: <Flag className="w-4 h-4" />,
};
...
export const EcosystemRelationshipManager: React.FC<EcosystemRelationshipManagerProps> = ({ preselectedSource }) => {
  const { contacts, events, communities, organizations, skills, projects, knowledge, relationships, addRelationship, removeRelationship, criticalPaths, activeCriticalPathId } = useEcosystemStore();
  ...
  const allDataItems: EcosystemDataItem[] = useMemo(() => {
    ...
    const activeCriticalPath = criticalPaths.find(p => p.id === activeCriticalPathId);
    ...
    if (activeCriticalPath) {
      items.push({ id: activeCriticalPath.id, name: activeCriticalPath.title, type: 'criticalPath' });
    }
    return items;
  }, [goals, interests, contacts, events, communities, organizations, skills, projects, knowledge, communityResources, criticalPaths, activeCriticalPathId]);
  const groupedDataItems = useMemo(() => {
    const groups: { [key: string]: EcosystemDataItem[] } = {
      'Critical Path': [], Goals: [], Interests: [], Contacts: [], Events: [], Communities: [], Organizations: [], Skills: [], Projects: [], Knowledge: [], 'Community Resources': [],
    };
    allDataItems.forEach(item => {
      if (item.type === 'criticalPath') groups['Critical Path'].push(item);
      else if (item.type === 'goal') groups.Goals.push(item);
