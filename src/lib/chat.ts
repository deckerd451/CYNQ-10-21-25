import type { Message, ChatState, ToolCall, WeatherResult, MCPResult, ErrorResult, SessionInfo, UserProfileContext, RelationshipSuggestion, ChatTakeaway } from '../../worker/types';
...
class ChatService {
  ...
  async sendMessage(
    message: string,
    context: UserProfileContext | undefined = undefined,
    ...
    try {
      const response = await fetch(`${this.baseUrl}/chat`, {
        ...
        body: JSON.stringify({ message, model, stream: !!onChunk, context, proactiveMode }),
      });
      if (!response.ok) {
  ...
  async summarizeSession(sessionId: string): Promise<{ success: boolean; data?: { title: string }; error?: string }> {
    try {
      const response = await fetch(`/api/chat/${sessionId}/summarize`, {
        method: 'POST',
      });
      return await response.json();
    } catch (error) {
      console.error('Failed to summarize session:', error);
      return { success: false, error: 'Failed to summarize session' };
    }
  }
  async getTakeaways(sessionId: string, context: UserProfileContext): Promise<{ success: boolean; data?: ChatTakeaway[]; error?: string }> {
    try {
      const response = await fetch(`/api/chat/${sessionId}/takeaways`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
      ...
      console.error('Failed to get takeaways:', error);
      return { success: false, error: error instanceof Error ? error.message : 'Failed to get takeaways' };
    }
  }

  async getRelationshipSuggestions(sessionId: string, context: UserProfileContext): Promise<{ success: boolean; data?: RelationshipSuggestion[]; error?: string }> {
    try {
      const response = await fetch(`/api/chat/${sessionId}/relationship-suggestions`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ context }),
      });
      ...
      return { success: false, error: error instanceof Error ? error.message : 'Failed to get relationship suggestions' };
    }
  }
}
