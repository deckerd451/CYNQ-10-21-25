...
import type { Message, ToolCall, UserProfileContext, RelationshipSuggestion, ChatTakeaway, Contact, KismetEvent, Community, Relationship, Organization, Skill, Project, KnowledgeItem, CommunityResource, Goal, CriticalPath } from './types';
...
function sanitizeJsonString(jsonString: string): string {
  ...
  // 1. Find the first '{' and the last '}' to extract the main JSON object.
  const startIndex = jsonString.indexOf('{');
  const endIndex = jsonString.lastIndexOf('}');

  if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) {
    return '{}'; // Not a valid-looking JSON object string
  }

  let sanitized = jsonString.substring(startIndex, endIndex + 1);

  // 2. Add quotes to unquoted keys.
  // This regex looks for an unquoted key (alphanumeric, _, or -) followed by a colon.
  sanitized = sanitized.replace(/([{,]\s*)(\w[\w\d_-]*)\s*:/g, '$1"$2":');

  // 3. Remove trailing commas before '}' or ']'.
  sanitized = sanitized.replace(/,\s*([}\]])/g, '$1');

  // 4. Attempt to close unclosed objects and arrays.
  ...
  // 5. Final check: if it's still not valid JSON, return an empty object string.
  try {
...
export class ChatHandler {
  ...
  private async generateToolResponse(
    ...
    let content = completion.choices[0]?.message?.content || 'Tool results processed successfully.';
    
    // Check if the tool result is an email draft action
    const emailDraftActionRegex = /\[DRAFT_EMAIL_ACTION: to=(.+), from=(.+), topic=(.+)\]/;
    const match = emailDraftActionRegex.exec(toolResults[0]?.result as string || '');

    if (match) {
        const [, to, from, topic] = match;
        content += `\n\n**Drafting Email:**\n\n**To:** ${to}\n**From:** ${from}\n**Subject:** Introduction: ${from} / ${to}\n\nHi ${to},\n\nI'd like to introduce you to ${from}. Based on your shared interest in ${topic}, I believe a connection would be mutually beneficial.\n\nI'll let you two take it from here!\n\nBest,\n[Your Name]`;
    }

    return content;
  }
  private buildSystemPrompt(context?: UserProfileContext, proactiveMode?: boolean): string {
    let prompt = "You are CYNQ, a helpful and insightful personal consultant. Your goal is to provide predictive and refined consultations based on the user's unique ecosystem and the broader community intelligence.";
    prompt += "\n\n# Available Tools:\n- `get_contact_email`: Retrieves a contact's stored email address.\n- `draft_email`: Prepares a draft for an introductory email between two contacts. When you use this tool, its result will be a special string. You should then take that string and compose the full email draft in your response to the user.";
    if (context) {
      prompt += "\n\n# User's Personal Context\nUse this to tailor your responses and provide personalized insights:";
      const activeCriticalPath = context.criticalPaths?.find(p => p.id === context.activeCriticalPathId);
      if (activeCriticalPath) {
        const cp = activeCriticalPath;
        prompt += `\n- **User's Active Critical Path: "${cp.title}"** (${cp.overallTimeline}). Description: ${cp.description}`;
        cp.phases.forEach(phase => {
          const taskStrings = phase.keyTasks.map(t => {
            let taskStr = `${t.text} [${t.completed ? 'Done' : 'Todo'}]`;
            if (t.assignedToOrgId) {
              const orgName = context.organizations?.find(o => o.id === t.assignedToOrgId)?.name;
              if (orgName) {
                taskStr += ` (Assigned to: ${orgName})`;
              }
            }
            return taskStr;
          });
          prompt += `\n  - Phase: ${phase.name} (${phase.duration}). Objective: ${phase.objective}. Tasks: ${taskStrings.join(', ')}`;
        });
      }
      ...
      if (context.relationships?.length) {
        ...
        const allItems: { id: string; name: string; type: string; }[] = [
          ...(context.criticalPaths?.map(cp => ({ id: cp.id, name: cp.title, type: 'criticalPath' })) || []),
  ...
  async generateRelationshipSuggestions(context: UserProfileContext): Promise<RelationshipSuggestion[]> {
    const allItems: { id: string; name: string; type: string; }[] = [
      ...(context.goals?.map((g) => ({ id: g.id, name: g.text, type: 'goal' })) || []),
      ...(context.interests?.map((interest, idx) => ({ id: `interest-${idx}`, name: interest, type: 'interest' })) || []),
      ...(context.contacts?.map((c) => ({ id: c.id, name: c.name, type: 'contact' })) || []),
    ...
    const existingLinks = new Set(context.relationships?.map(r => [r.sourceId, r.targetId].sort().join('--')));

    const suggestionPrompt = `
      TASK: Analyze the user's ecosystem and suggest 2-3 new, valuable relationships between existing items.
      CONTEXT: The user has the following items in their ecosystem:
      ${allItems.map(item => `- [${item.type}] ${item.name} (id: ${item.id})`).join('\n')}
      
      EXISTING RELATIONSHIPS TO IGNORE:
      ${[...(existingLinks)].join('\n') || 'None'}

      INSTRUCTIONS:
      1. Identify potential new connections between items that are not yet linked.
      2. Prioritize non-obvious but strategic links (e.g., a 'skill' to a 'project', a 'contact' to a 'goal').
      3. For each suggestion, provide a brief, compelling justification (max 15 words).
      4. **RESPONSE FORMAT:** Respond ONLY with a single, valid JSON object. Do not include any other text. The object must have a key "suggestions" containing an array of objects.
      
      JSON STRUCTURE:
      {
        "suggestions": [
          { "sourceId": "string", "sourceType": "string", "targetId": "string", "targetType": "string", "justification": "string" }
        ]
      }
    `;
    ...
    try {
      ...
      const endIndex = jsonString.lastIndexOf('}');
      if (startIndex === -1 || endIndex === -1 || endIndex < startIndex) {
        console.error("Invalid JSON structure in relationship suggestions:", responseContent);
        return [];
      }
      jsonString = jsonString.substring(startIndex, endIndex + 1);
      const sanitizedJsonString = sanitizeJsonString(jsonString);
      const parsed = JSON.parse(sanitizedJsonString);
      return parsed?.suggestions && Array.isArray(parsed.suggestions) ? parsed.suggestions : [];
    } catch (error) {
      console.error("Failed to parse relationship suggestions:", error, { response: responseContent });
      return [];
    }
  }
}