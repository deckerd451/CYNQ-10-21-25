import type { WeatherResult, ErrorResult, UserProfileContext, Contact } from './types';
...
const customTools = [
  ...
  },
  {
    ...
    function: {
        ...
        description: "Retrieves the email address for a specific contact in the user's ecosystem.",
        parameters: {
            type: 'object',
            properties: {
                contact_name: { type: 'string', description: "The full name of the contact to find." }
            },
  ...
  },
  {
    ...
    function: {
        name: 'draft_email',
        description: "Prepares a draft for an introductory email between two contacts from the user's ecosystem.",
        parameters: {
            type: 'object',
            properties: {
                to_contact_name: { type: 'string', description: "The full name of the contact receiving the email." },
                from_contact_name: { type: 'string', description: "The full name of the contact being introduced." },
                topic: { type: 'string', description: "The shared interest or topic for the introduction." }
            },
            required: ['to_contact_name', 'from_contact_name', 'topic']
        }
    }
  }
];
...
export async function executeTool(name: string, args: Record<string, unknown>, context?: UserProfileContext): Promise<ToolResult> {
  try {
    switch (name) {
      case 'get_weather':
        return {
      ...
      case 'get_contact_email': {
        ...
        const contact = context?.contacts?.find((c: Contact) => c.name.toLowerCase() === contactName.toLowerCase());
        if (contact && contact.email) {
            return { content: `The email for ${contact.name} is ${contact.email}.` };
        }
        return { content: `I could not find an email for a contact named ${contactName}.` };
      }
      case 'draft_email': {
        const { to_contact_name, from_contact_name, topic } = args;
        if (!to_contact_name || !from_contact_name || !topic) {
            return { error: 'All parameters (to_contact_name, from_contact_name, topic) are required.' };
        }
        return { content: `[DRAFT_EMAIL_ACTION: to=${to_contact_name}, from=${from_contact_name}, topic=${topic}]` };
      }
      default: {
